...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      setUnreadCount(Math.floor(Math.random() * 8) + 2);
      ...
      <div className="fixed top-4 right-4 z-[60] flex items-center space-x-3">
        ...
        {isAuthenticated && (
          <button
            ...
            className="relative bg-slate-800/90 backdrop-blur-sm text-slate-200 p-3 rounded-xl shadow-xl hover:shadow-2xl hover:bg-slate-700 transition-all duration-300 border border-slate-700"
          >
        ...
        <button
          ...
          className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white px-4 py-3 rounded-xl shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 flex items-center space-x-2 group"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
